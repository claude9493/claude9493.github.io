---
layout: page
title: About
sidebar_link: true
---
<h1>About Me and My Blog</h1>
<p class="message">
  Hello, I am <font face="Bradley Hand ITC">Yun Zhang</font>, a Statistics student from <a href="http://sustc.edu.cn/en/">SUSTech</a>.<br>
  Here is my blog.<br>
  I use git pages and fork a theme.<br>
  I have written several notes and articles about experiance and troubles encountered in learning.
  I also wrote some other blog basing on my interest and research.<br>
  Thank you.<br>
  Following is my photo, taken in the spring of 2017.<br/>
  </p>

  ![](/images/ME.jpg)

<p class="message">
  Hello, 这里是张昀的博客. 本人目前在南科大统计与数据科学系统计专业读大四, 这个博客是用 Gitpage 实现的, 目前写了几篇笔记和心得, 关于学习中遇到的问题和几个我   感兴趣的话题. 上面那张是我的照片, 2017年春天拍的.<br>
  </p>
  <blockquote><font face = "STFangsong">
    大雨落幽燕, 白浪滔天, 秦皇岛外打鱼船. 一片汪洋都不见, 知向谁边?<br>
    往事越千年, 魏武挥鞭, 东临碣石有遗篇. 萧瑟秋风今又是, 换了人间.
  </font></blockquote>
<br>

<h1>Website Information and Thanks</h1>
<p class="message">
 I bought this domain(<s>claude9493.name</s> claude9493.com) from <s>Wanwang</s> Tencent Cloud. And I use git page as my server.<br/>
 I use the Jekyll theme Hydeout as my blog's basic framework. This beautiful and useful theme is created by Andrew Fong.<br>
 I store my figures and pictures in <a href="https://www.qiniu.com/">Qiniu</a>, It provided a large storage freely for user.<br>
 Thanks those who are still writing blogs, encouraging me to start a blog.<br/>
 Also thanks Github providing gitpages, making me not depressed with the high cost of server.<br/>
 Thanks you as well. Thanks for noticing me.<br/>
</p>
<br>


